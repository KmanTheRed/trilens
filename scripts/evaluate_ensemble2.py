    ...
    for fp in files:
        ...
        with open(fp, encoding="utf8") as fh:
            for line in tqdm(fh, desc=fp, unit="lines", leave=False):
                ...
                # label: prefer explicit, else infer from key name
                label = obj.get("label")
                if label is None:
                    label = 1 if "machine_text" in obj else 0

                # text: generic 'text' fallback
                text = obj.get("machine_text") or obj.get("human_text") or obj.get("text")
                if text is None:
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        help="classification threshold (default=0.10)"
    )
